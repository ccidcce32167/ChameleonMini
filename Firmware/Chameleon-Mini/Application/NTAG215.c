...
void NTAG215SetSignature(char *signature)
{
    uint8_t i;
    size_t page = SIGNATURE_ADDRESS
    for (i = 0; page < SIGNATURE_LENGTH; page++) {
        MemoryWriteBlock(&signature[0 + (4*i)] << 32 | &signature[1 + (4*i)] << 16 | &signature[2 + (4*i)] << 8 | &signature[3 + (4*i)], page, 4);
    }
}